...
class IMFSLStaffSupabaseUser extends BaseAuthUser {
  IMFSLStaffSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> iMFSLStaffSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = IMFSLStaffSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
